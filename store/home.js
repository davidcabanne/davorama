import universeSimulation from "../public/images/categories/universeSimulation.jpg";
import electricity from "../public/images/categories/electricity.jpg";
import glassBox from "../public/images/categories/glassBox.jpg";
import temporalDrift from "../public/images/categories/temporalDrift.jpg";
import theThinBlueLie from "../public/images/categories/theThinBlueLie.jpg";
import capitalism from "../public/images/categories/capitalism.png";
import film from "../public/images/categories/london2024.jpg";

const categories = [
  {
    name: "moving through space and time",
    img: universeSimulation,
    link: "/moving-through-space-and-time",
  },
  { name: "Electricity", img: electricity, link: "/electricity" },
  { name: "Glass Box", img: glassBox, link: "/glass-box" },
  { name: "Temporal Drift", img: temporalDrift, link: "/temporal-drift" },
  {
    name: "The Thin Blue Lie",
    img: theThinBlueLie,
    link: "/the-thin-blue-line",
  },
  {
    name: "capitalism",
    img: capitalism,
    link: "/capitalism",
  },
  {
    name: "films",
    img: film,
    link: "/films-all",
  },
];

export { categories };
